            ...
            >
                ...
                >
                    ...
                    <View style={styles.header}>
                        <Text style={[styles.title, { color: theme.text }]}>{t('welcomeBackTitle')}</Text>
                        <Text style={[styles.subtitle, { color: theme.textSecondary }]}>{t('enterSanctuaryKeys')}</Text>
                    </View>

                    <View style={[styles.methodSelector, { backgroundColor: theme.surfaceSecondary }]}>
                        <Animated.View style={[styles.activeIndicator, { backgroundColor: theme.primary, transform: [{ translateX: tabTranslateX }] }]} />
                        <TouchableOpacity
                            style={styles.methodTab}
                            onPress={() => setLoginMethod('password')}
                            activeOpacity={1}
                        >
                            <Text style={[styles.methodText, { color: theme.textSecondary }, loginMethod === 'password' && { color: isDark ? '#000' : '#FFF', fontWeight: 'bold' }]}>{t('password')}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                            style={styles.methodTab}
                            ...
                            activeOpacity={1}
                        >
                            <Text style={[styles.methodText, { color: theme.textSecondary }, loginMethod === 'pin' && { color: isDark ? '#000' : '#FFF', fontWeight: 'bold' }]}>{t('safetyPin')}</Text>
                        </TouchableOpacity>
                    </View>
                    ...
                    <Animated.View style={[styles.formWrapper, { opacity: fadeAnim }]}>
                        {loginMethod === 'password' ? (
                            <View style={styles.formGroup}>
                                <View style={styles.inputGroup}>
                                    <Text style={[styles.miniLabel, { color: theme.textSecondary }]}>{t('username')}</Text>
                            ...
                            <View style={styles.formGroup}>
                                ...
                                <Text style={[styles.label, { color: theme.text }]}>{t('enter6DigitPin')}</Text>
                                <View style={styles.pinGridContainer}>
                                    <TextInput
                                        ...
                                        maxLength={6}
                                        value={form.pin}
                                        onChangeText={(val) => setForm(f => ({ ...f, pin: val }))}
                                        autoFocus={loginMethod === 'pin'}
                                    />
                                    {[0, 1, 2, 3, 4, 5].map((idx) => {
                                        const char = form.pin[idx] || '';
                                        const isFocused = form.pin.length === idx;
                                        return (
                                            <View
                                                key={idx}
                                                style={[
                                                    ...
                                                    isFocused && [styles.pinBoxActive, { borderColor: theme.primary }],
                                                    char ? [styles.pinBoxFilled, { borderColor: theme.primary }] : null
                                                ]}
                                            >
                                                <Text style={[styles.pinText, { color: theme.text }]}>{char ? '•' : ''}</Text>
                                            </View>
                                        );
                                    })}
                                </View>
                                <Text style={[styles.pinHint, { color: theme.textSecondary }]}>{t('pinHint')}</Text>
                            </View>
                        )}
                        ...
                        >
                            <Text style={[styles.forgotText, { color: theme.textSecondary }]}>
                                {t('problemsEntering')}<Text style={[styles.recoverText, { color: theme.primary }]}>{t('recoverAccount')}</Text>
                            </Text>
                        </TouchableOpacity>

                        <View style={styles.footer}>
                            <Text style={[styles.footerText, { color: theme.textSecondary }]}>{t('newToNivora')}</Text>
                            <TouchableOpacity onPress={() => router.push('/auth/signup')}>
                                <Text style={[styles.signupLinkText, { color: theme.primary }]}>{t('signUp')}</Text>
                            </TouchableOpacity>
                        </View>
                    </Animated.View>
                </ScrollView>
